...
file1 = "radiomics__CTCOVID__imageALL_binary.csv"
# file1 = "trainWOrih.csv"
f = open(file1)
csv_f = csv.reader(f)
features = next(csv_f)

dataset = pd.read_csv(file1, names=features, skiprows=1, low_memory=False)
dataset["Grade-binary"] = pd.to_numeric(dataset["Grade-binary"], errors='coerce')
dataset.dropna(axis=1, thresh=2, inplace=True)
# dataset.dropna(inplace=True)
arrayRad = dataset.values
X = arrayRad[:,1:1581]
Y = arrayRad[:,1581]
...
X_validation = validation[:,1:1581]
X_validation = X_validation.astype('float64')
Y_validation = validation[:,1581]
...
for name, model in models:
	for kind, selection in sel:
		...
		featNum = 1
		for val in feat:
			if(features[featNum] not in features_dict.keys()):
				features_dict[features[featNum]] = 0
			if val == True:
				features_dict[features[featNum]]+=1
			featNum+=1
...
output.write("Accuracy: " + repr(accuracy_score(Y_test, Y_pred)))
output.write("Average Precision Score: " + repr(average_precision_score(Y_test, Y_pred)))
output.write("Kappa: " + repr(cohen_kappa_score(Y_test, Y_pred)))
output.write("Hamming Loss: " + repr(hamming_loss(Y_test, Y_pred)))
output.write("AUC: " + repr(roc_auc_score(Y_test, Y_pred)))
output.write("Sensitivity: " + repr(recall_score(Y_test, Y_pred)))
